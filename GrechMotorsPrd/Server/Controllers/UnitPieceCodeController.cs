...
namespace GrechMotorsPrd.Server.Controllers
{
    ...
    public class UnitPieceCodeController : ControllerBase
    {
        ...
        [HttpGet]
        [Route("getPiecesQrIdentificationCodesByUnit/{unit_id}")]
        public async Task<ActionResult<List<string>>> GetPiecesQrIdentificationCodesByUnit(int unit_id)
        {
            // Retrieve all qr_code_numbers from the units in the database that match the provided unit_id
            var qrCodeNumbers = await _context.unitspiecescodes
